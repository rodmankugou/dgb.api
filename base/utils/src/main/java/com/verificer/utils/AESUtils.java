...
public class AESUtils {
    ...
    /**
     * AES 加密
     * @param key 长度32位的字符串，建议使用Guid生成
     * @param input 需要对称加密的内容
     * @return
    ...
    public static String encrypt(String key, String input) throws GeneralSecurityException,
        ...
        byte[] keyBs = key.getBytes("UTF-8");
        byte[] inputBs = input.getBytes("UTF-8");
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
        SecretKeySpec keySpec = new SecretKeySpec(keyBs, "AES");
        // CBC模式需要生成一个16 bytes的initialization vector:
        SecureRandom sr = SecureRandom.getInstanceStrong();
        byte[] iv = sr.generateSeed(16);
        IvParameterSpec ivps = new IvParameterSpec(iv);
        cipher.init(Cipher.ENCRYPT_MODE, keySpec, ivps);
        byte[] data = cipher.doFinal(inputBs);
        // IV不需要保密，把IV和密文一起返回:
    ...
    public static String decrypt(String key, String  input) throws GeneralSecurityException,
            UnsupportedEncodingException {
        byte[] keyBs = key.getBytes("UTF-8");
        byte[] inputBs = Base64.getDecoder().decode(input);

        // 把input分割成IV和密文:
        byte[] iv = new byte[16];
        byte[] data = new byte[inputBs.length - 16];
        System.arraycopy(inputBs, 0, iv, 0, 16);
        System.arraycopy(inputBs, 16, data, 0, data.length);
        // 解密:
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
        SecretKeySpec keySpec = new SecretKeySpec(keyBs, "AES");
        IvParameterSpec ivps = new IvParameterSpec(iv);
